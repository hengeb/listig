...
**Proactive sync on opening the archive (`Archive/ArchiveSynchronizer.php`)** — the lazy, open-one-message reconciliation above only ever catches a single mail going missing, and only once someone opens it. `ArchiveController::index()` (the table/thread view) additionally calls `ArchiveSynchronizer::sync($list)` — throttled to once per `ArchiveController::SYNC_INTERVAL_SECONDS` (5 minutes) per list *per session* (`$_SESSION['archive_synced'][$list->name]`, checked/set by `ArchiveController::syncIfDue()`) — to catch mail added or removed directly on the IMAP archive folder outside Listig entirely (an operator moving/deleting a message by hand, another mail client touching the folder), not just mail Listig itself already knows might be gone.

`sync()` always pays for one `SEARCH ALL` + `FETCH OVERVIEW` scan of the whole archive folder (Message-ID per message, no body — same call shape as `findUidByMessageId()` below, including the same `$disableServerEncoding = true` workaround) to build the current set of Message-IDs actually on IMAP, then diffs it against `SELECT message_id FROM archived_mail WHERE list_cn = ...` via `array_diff_key()` in both directions. This is deliberately a Message-ID *set* diff, not a `COUNT(*)`/`STATUS (MESSAGES)` comparison: a cheap count-only check cannot distinguish "nothing changed" from "one mail was deleted and a different one was added" (same total either way), which is exactly the case this needs to handle correctly. Only the two diffs are ever fetched further: a Message-ID present on IMAP but missing from `archived_mail` gets one full `getMail()` fetch, then `ArchiveIndexer::index()` (populating subject/sender/date/attachments, same as a normal distribute); a Message-ID indexed but no longer on IMAP is just `ArchiveIndexer::remove()`d, no fetch at all. A mail unchanged on both sides costs nothing beyond appearing once in the initial overview scan — this never re-fetches or re-indexes something already correctly indexed.
...
**Deep-link redirect-back** — unlike the magic-link flow (see the archive viewer's own login-required page, which has "no return-URL redirect-back... no 'next' concept to hook into"), OIDC *does* support returning to the originally-requested page, since the whole round trip to the IdP and back happens within one continuous session (the magic-link flow, by contrast, is deliberately interrupted — the user leaves the browser entirely to click a link in their mail client). `AuthMiddleware` is where this starts: when it intercepts an unauthenticated request and `'oidc.enabled'` is true (constructor arg, wired in `public/index.php`), it skips `/_/login`'s form entirely and redirects straight to `/_/login/oidc?next={urlencoded current path+query}` — a user hitting a bookmarked/shared deep link (e.g. `/mylist`) goes directly to the IdP, never sees the login form, and lands back on `/mylist` after authenticating. Without OIDC configured, `AuthMiddleware` falls back to the plain `/_/login` redirect exactly as before (the magic-link flow has nothing to attach a "next" to).

`AuthController::loginOidc()` carries `next` across the round trip via `$_SESSION['oidc_next']` — not a query param appended to the IdP's own authorization URL, since the IdP fully controls what it appends to its registered `redirect_uri` on the way back (no room for one of our own), whereas the session cookie survives the round trip for free (same browser, same `PHPSESSID`). Set only on the *initial* leg, only when a `next` param is actually present (a plain visit to `/_/login/oidc`, e.g. the login page's own SSO button, never sets it); read and cleared on the *callback* leg, falling back to `/` if absent — exactly the pre-existing behavior for a non-deep-link OIDC login. `AuthController::sanitizeNext()` is the gate: `next` ultimately originates from a query string an attacker fully controls (a crafted deep link to this app), so only a same-origin relative path is ever accepted — rejects an empty value, anything not starting with a single `/` (a full URL), a scheme-relative `//evil.example` (browsers resolve that as `https://evil.example`, not a path), a backslash-led `/\evil.example` (some browsers normalize `\` to `/`), and — as defense in depth on top of the prefix check — anything `parse_url()` can still extract a scheme or host from.
